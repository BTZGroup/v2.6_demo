...
const $httpMultiPart = function(options = {}) {
  ...
  if (options.logInfo && Object.keys(options.logInfo).length) {
    BSConfig.microService.log && Service.logger.save(options.logInfo);
...
export const $httpXMLInstance = function xhrRequest({ url, method = "GET", data, headers, logInfo, isAsync = false }) {
  ...
  if (logInfo && Object.keys(logInfo).length) {
    BSConfig.microService.log && Service.logger.save(logInfo);
  }
...
export const $httpRequest = function({ url, type, data, method, headers, logInfo, cancelHttp = null, timeout }) {
  let options = {};
  const curVersion = process.env.PVERSION;
  const token = Utils.getCache("token", "session");
  const defaultHeader = {};
  if (token) {
    defaultHeader.Authorization = (url.indexOf('lingtong') >= 0 ) ? "Bearer " + token : url.indexOf('gbaccess') >= 0 ? 'Bearer ' + token : token;
  }
  options.url = url + (curVersion ? "?" + curVersion : "");
